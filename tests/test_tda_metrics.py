...
def test_topology_snapshot_empty_window(config: Config) -> None:
...
def test_topology_snapshot_single_point(config: Config) -> None:
...
def test_cloud_diameter_nonnegative(n_points: int) -> None:
    ...
    result = topology_snapshot(
        pts,
        config=Config(pca_components=8, tda_enabled=False),
    )
    assert result.diameter >= 0.0


def test_persistence_l1_integrated(config: Config) -> None:
...
def test_risk_score_normalization(config: Config) -> None:
    safe_metrics = {
        "cosine_cont": 0.98,
        "lipschitz": 0.05,
        "cloud_diameter": 5.0,
