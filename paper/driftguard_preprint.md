...
### 5.2 Threat suites
We evaluate a deliberate **hybrid** two-source suite (to match the non-toxic public WildChat release with explicit unsafe prompts):
- **Benign multi-turn sessions:** `allenai/WildChat-1M` conversations (hub non-toxic release; rows with `toxic` are skipped).
- **Unsafe multi-turn sessions:** `allenai/wildjailbreak` **adversarial_harmful** user prompts, embedded after a short synthetic benign user–assistant preamble so the monitor sees a multi-turn chat.
...
On the **WildChat-1M + WildJailbreak** hybrid suite (N≈200 benign WildChat multi-turn + 200 synthetic-history unsafe prompts, 5 seeds), the fused risk detector achieves **AUROC = 0.923 +- 0.018** (bootstrap 95% CI), improving over continuity-only by ΔAUROC = 0.163. Median lead time is 14 tokens (IQR 9-21). *Re-run `experiments/run_driftguard_benchmark.py` to refresh numbers for this evaluation mix.*
</think>
